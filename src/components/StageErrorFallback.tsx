  ...
  return (
    <div className="flex flex-col h-full w-full bg-bg-primary p-6 overflow-auto">
      <div className="max-w-2xl mx-auto w-full space-y-6">
        <div className="bg-error bg-opacity-10 border border-error border-opacity-35 rounded-xl p-6 shadow-lg border-t-4 border-t-error">
            ...
            <div className="space-y-2">
              {stageErrors.map((err) => (
                <div 
                  ...
                  className="bg-bg-tertiary hover:bg-bg-secondary border border-border-primary hover:border-error hover:border-opacity-40 transition-all rounded-lg p-4 cursor-pointer flex flex-col space-y-2 group shadow-sm"
                >
                  <div className="flex items-center justify-between text-xs font-mono">
                    <span className="px-2 py-0.5 rounded font-bold uppercase bg-error bg-opacity-15 text-error border border-error border-opacity-35 text-[9px]">
                      {err.severity}
                    </span>
                    {err.range && (
