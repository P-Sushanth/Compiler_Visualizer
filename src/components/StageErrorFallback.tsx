...
export function StageErrorFallback({
  ...
  const stageErrors = diagnostics.filter(
    (err) => err.stage === stage && err.severity === 'error',
  )

  // Find which stage actually failed
  const failedStage =
    diagnostics.find((err) => err.severity === 'error')?.stage || 'unknown'
      ...
      <div className="max-w-2xl mx-auto w-full space-y-6">
        ...
        {failedStage === stage && stageErrors.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-xs uppercase tracking-wider text-text-muted font-bold font-mono">
              Errors Found ({stageErrors.length})
            </h4>
