...
export function ConsolePanel() {
  ...
  const lastCompileDurationMs = useCompilerStore(
    (state) => state.lastCompileDurationMs,
  )
  ...
  const getLogs = () => {
    ...
    if (status === 'idle') {
      logs.push(
        'Compiler visualizer idle. Write some code in main.c to compile.',
      )
      return logs
    }
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      if (duration !== null) {
        logs.push(
          `[${stage.toUpperCase()}] Stage completed successfully in ${duration.toFixed(2)}ms`,
        )
      } else {
    ...
    if (status === 'success') {
      logs.push(
        `Compilation succeeded! Total duration: ${lastCompileDurationMs?.toFixed(2) || '0.00'}ms`,
      )
    } else if (status === 'error') {
      logs.push(
        'Compilation failed. Please resolve the issues in the Problems tab.',
      )
    }
        ...
        {activeConsoleTab === 'problems' ? (
          diagnostics.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-text-muted text-xs space-y-2">
              <svg
                className="w-8 h-8 text-success opacity-60"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <div
                ...
                className={
                  log.startsWith('Compilation succeeded')
                    ...
                    : log.startsWith('Compilation failed') ||
                        log.includes('failed')
