...
describe('Sidebar Component', () => {
  ...
  it('renders all stage navigation buttons', () => {
    ...
    expect(
      screen.getByRole('navigation', { name: /compiler stages navigation/i }),
    ).toBeInTheDocument()
  ...
  it('highlights the active stage button', () => {
    ...
    const lexButton = screen.getByRole('button', {
      name: /view lexical analysis stage/i,
    })
    expect(lexButton).toHaveAttribute('aria-pressed', 'true')

    const astButton = screen.getByRole('button', {
      name: /view syntax analysis \(ast\) stage/i,
    })
