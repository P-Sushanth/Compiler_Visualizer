...
class Parser {
  ...
  private consume(typeOrValue: string, message: string): Token {
    if (!this.isAtEnd() && (this.peek().type === typeOrValue || this.peek().value === typeOrValue)) {
      return this.advance();
    }
  ...
  public parse(): ParserOutput {
    ...
    this.tokens = this.tokens.filter(t => t.type !== 'whitespace' && t.type !== 'comment');
    ...
    const programNode: ProgramNode = {
      id: programId,
      type: 'Program',
      range: startToken ? { start: startToken.range.start, end: this.previous()?.range.end || startToken.range.end } : { start: { line: 0, column: 0, offset: 0 }, end: { line: 0, column: 0, offset: 0 } },
      children: body,
      body
    };
  ...
  private expressionStatement(): string {
    ...
    const node: ExpressionStatementNode = {
      ...
      expression: expr,
      range: { start: this.nodes[expr].range.start, end: this.previous().range.end },
      children: [expr]
    };
    return this.addNode(node);
  }
  ...
  private assignment(): string {
    ...
    if (this.match('=', '+=', '-=')) {
      ...
      const node: BinaryExpressionNode = {
        ...
        left: expr,
        right: value,
        range: { start: this.nodes[expr].range.start, end: this.nodes[value].range.end },
        children: [expr, value]
      };
      return this.addNode(node);
    }
  ...
  private equality(): string {
    ...
    while (this.match('==', '!=')) {
      ...
      const node: BinaryExpressionNode = {
        ...
        left: expr,
        right,
        range: { start: this.nodes[expr].range.start, end: this.nodes[right].range.end },
        children: [expr, right]
      };
