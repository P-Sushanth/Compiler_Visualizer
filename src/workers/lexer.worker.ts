...
import type { Token, TokenType, CompilerDiagnostic, SourcePosition } from '../types/compiler';

const KEYWORDS = new Set([
  'int', 'return', 'if', 'else', 'while', 'for', 'void', 'char', 'float', 'double', 'break', 'continue'
]);
...
export function lex(sourceCode: string): { tokens: Token[], errors: CompilerDiagnostic[] } {
  ...
  while (offset < sourceCode.length) {
    ...
    if (char === '/' && sourceCode[offset + 1] === '*') {
      let value = '/*';
      advance(2);
      while (offset < sourceCode.length && !(sourceCode[offset] === '*' && sourceCode[offset + 1] === '/')) {
        value += sourceCode[offset];
        advance();
      }
    ...
    if (/[a-zA-Z_]/.test(char)) {
      let value = '';
      while (offset < sourceCode.length && /[a-zA-Z0-9_]/.test(sourceCode[offset])) {
        value += sourceCode[offset];
        advance();
      }
    ...
    if (/[0-9]/.test(char)) {
      ...
      while (offset < sourceCode.length && (/[0-9]/.test(sourceCode[offset]) || (sourceCode[offset] === '.' && !hasDecimal))) {
