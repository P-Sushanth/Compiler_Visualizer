...
export class AssemblyGenerator {
  ...
  private getRegister(temp: string): string {
    if (!this.regMap[temp]) {
      this.regMap[temp] =
        this.availableRegs[this.regIndex % this.availableRegs.length]
      this.regIndex++
    }
        ...
        switch (inst.opcode) {
          case 'LOAD_CONST':
            this.emit(
              'movl',
              [
                this.getOperand(inst.operands[0]),
                this.getRegister(inst.result!),
              ],
              'Load constant',
              srcNode,
            )
          ...
          case 'LOAD':
            this.emit(
              'movl',
              [
                this.getRegister(inst.operands[0]),
                this.getRegister(inst.result!),
              ],
              'Load value',
              srcNode,
          ...
          case 'ADD':
            this.emit(
              'movl',
              [this.getOperand(inst.operands[0]), '%eax'],
              'Load left operand',
              srcNode,
            )
            this.emit(
              'addl',
              [this.getOperand(inst.operands[1]), '%eax'],
              'Add right operand',
              srcNode,
            )
            this.emit(
              'movl',
              ['%eax', this.getRegister(inst.result!)],
              'Store result',
              srcNode,
            )
            break
          ...
          case 'DIV':
            ...
            this.emit(
              'idivl',
              [this.getOperand(inst.operands[1])],
              'Divide',
              srcNode,
            )
            this.emit(
          ...
          case 'GTE': {
            ...
            this.emit(
              'movzbl',
              ['%al', this.getRegister(inst.result!)],
              'Zero extend to 32bit',
              srcNode,
            )
            break
          }
          ...
          case 'JUMPIFNOT':
            this.emit(
              'cmpl',
              ['$0', this.getOperand(inst.operands[0])],
              'Check if false',
              srcNode,
            )
