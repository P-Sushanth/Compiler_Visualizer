...
describe('Compiler Stress & Performance Test (Large File)', () => {
  it('should compile a large C source file with many statements without crashing', () => {
    ...
    console.log(`Lexer processed ${tokens.length} tokens in ${lexDuration.toFixed(2)}ms`);
    ...
    console.log(`Parser parsed ${Object.keys(ast.nodes).length} nodes in ${parseDuration.toFixed(2)}ms`);
    ...
    console.log(`Semantic Analyzer processed ${Object.keys(semanticModel.symbols).length} symbols in ${semanticDuration.toFixed(2)}ms`);
    ...
    console.log(`IR Generator generated ${Object.keys(ir.instructions).length} instructions in ${irDuration.toFixed(2)}ms`);
    ...
    const totalDuration = lexDuration + parseDuration + semanticDuration + irDuration;
