...
export type IRLayoutOutput = {
  ...
  edges: Array<{ id: string; source: string; target: string; animated?: boolean; label?: string; style?: any }>;
};
...
self.onmessage = (event: MessageEvent<WorkerRequest<IRProgram>>) => {
  ...
  Object.values(ir.blocks).forEach((block) => {
    ...
    const instructionsData = block.instructionIds.map(id => ir.instructions[id]);
