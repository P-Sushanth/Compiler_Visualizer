    ...
    <div className="flex flex-col h-full w-full bg-bg-primary font-mono text-sm">
      ...
      {isBeginner && (
        <div className="p-4 bg-bg-tertiary border-b border-border-primary text-xs leading-relaxed text-text-secondary shrink-0">
          ...
          <p>
            The <strong>Optimizer</strong> processes the intermediate code (IR)
            to make it run faster or use less memory without changing what the
            program actually does. It applies sequential <strong>passes</strong>{' '}
            such as
            <em> Constant Folding</em> (replacing expressions like{' '}
            <code>10 + 20</code> with <code>30</code>) and
            <em> Dead Code Elimination</em> (removing code that executes but has
            no effect, like unused variables).
          </p>
        </div>
      )}
          ...
          <div className="space-y-4 max-w-3xl mx-auto">
            {passes.length > 0 ? (
              passes.map((pass, index) => (
                <div
                  key={pass.id}
                  className="bg-bg-tertiary border border-border-primary rounded p-4 relative overflow-hidden"
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-text-primary flex items-center space-x-2">
                      <span className="bg-bg-secondary text-text-muted px-2 py-0.5 rounded text-xs">
                        #{index + 1}
                      </span>
                      <span>{pass.name}</span>
                    </h3>
              ...
              <div className="flex flex-col items-center justify-center p-12 border border-dashed border-border-primary rounded text-text-muted">
                ...
                <p className="text-xs mt-2">
                  Try writing code with dead variables or constant expressions
                  like <code>int x = 5 + 5;</code>
                </p>
              </div>
            )}
          </div>
