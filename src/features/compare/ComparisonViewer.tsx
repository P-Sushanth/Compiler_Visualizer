...
export function ComparisonViewer() {
  ...
  const getExplanation = () => {
    ...
    if (activeTab === 'ast-ir') {
      if (hoveredNodeId && ast.nodes[hoveredNodeId]) {
        const node = ast.nodes[hoveredNodeId]
        const matchingIR = irInstructions.filter(
          (inst) => inst.sourceNodeId === hoveredNodeId,
        )
    ...
    if (activeTab === 'ir-assembly') {
      if (hoveredNodeId) {
        ...
        const matchingASM = assembly.filter(
          (inst) => inst.sourceNodeId === hoveredNodeId,
        )
            ...
            {activeTab === 'source-ast' && (
              <div className="space-y-0.5">
                {sourceLines.map((line, idx) => {
                  ...
                  const isNodeMapped =
                    hoveredNodeId &&
                    matchingNodes.some((n) => n.id === hoveredNodeId)
                  const highlightClass =
                    isLineHovered || isNodeMapped
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={() => {
                        ...
                        if (matchingNodes.length > 0) {
                          setHoveredNodeId(
                            matchingNodes[matchingNodes.length - 1].id,
                          )
                        }
                      }}
                      onMouseLeave={() => {
                    ...
                    >
                      <div className="w-8 text-text-muted text-right pr-3 select-none">
                        {lineNum}
                      </div>
                    ...
                    >
                      ...
                      {node.range && (
                        <div className="text-[9px] text-text-muted mt-1">
                          Line {node.range.start.line}:{node.range.start.column}{' '}
            ...
            {activeTab === 'ir-assembly' && (
              <div className="space-y-1">
                {irInstructions.map((inst) => {
                  const isInstHovered =
                    hoveredNodeId && inst.sourceNodeId === hoveredNodeId
            ...
            {activeTab === 'source-ast' && (
              <div className="space-y-1">
                {Object.values(ast.nodes).map((node) => {
                  const isNodeHovered = hoveredNodeId === node.id
                  const matchingNodes =
                    hoveredLine !== null ? getASTNodesForLine(hoveredLine) : []
                  const isLineRelated = matchingNodes.some(
                    (n) => n.id === node.id,
                  )

                  const highlightClass =
                    isNodeHovered || isLineRelated
                      ? 'bg-info/20 border border-info text-text-primary font-bold shadow-sm'
                      : 'bg-bg-tertiary border border-border-primary/40 hover:bg-bg-secondary text-text-secondary'
            ...
            {activeTab === 'ir-assembly' && (
              <div className="space-y-0.5">
                {assembly.map((inst, idx) => {
                  const isLabel =
                    inst.op.endsWith(':') || inst.op.startsWith('.')
                  const isAsmHovered =
                    hoveredNodeId && inst.sourceNodeId === hoveredNodeId
                  ...
                  return (
                    <div
                      key={inst.id}
                      onMouseEnter={() =>
                        setHoveredNodeId(inst.sourceNodeId ?? null)
                      }
                      onMouseLeave={() => setHoveredNodeId(null)}
      ...
      {isBeginner && (
        <div className="p-4 bg-bg-tertiary text-xs leading-relaxed text-text-secondary border-t border-border-primary shrink-0 z-10">
          <h3 className="font-bold text-info mb-1 uppercase tracking-wider text-[10px]">
            {activeTab === 'source-ast' &&
              'Flow 1: Syntactic Parser Transformation'}
            {activeTab === 'ast-ir' && 'Flow 2: Intermediate Code Flattening'}
            {activeTab === 'ir-assembly' &&
              'Flow 3: Low-Level Register Code Generation'}
          </h3>
