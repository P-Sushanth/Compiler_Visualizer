...
const AST_GLOSSARY: Record<string, string> = {
  ...
  FunctionDeclaration:
    'Defines a callable function with a return type, name, and parameters.',
  BlockStatement:
    'A block of statements grouped together inside curly braces { }.',
  ReturnStatement:
    'Terminates function execution and returns a value to the caller.',
  VariableDeclaration:
    'Declares a variable name, type, and optional initial value.',
  BinaryExpression:
    'An operation combining two values (e.g. addition, subtraction).',
  IfStatement:
    'A conditional check executing branches based on a boolean condition.',
  ForStatement:
    'A loop repeating execution block until a condition becomes false.',
  ExpressionStatement:
    'A statement consisting of a single evaluated expression.',
...
export function ASTNodeUI({
  ...
  if (data.type === 'error') {
    ...
    data.type === 'function' ||
    data.type === 'Program' ||
    data.type === 'FunctionDeclaration'
  ) {
    bgColor = 'bg-[#2A3140] border-info/50'
  } else if (
    data.type === 'statement' ||
    data.type === 'BlockStatement' ||
    data.type === 'ReturnStatement' ||
    data.type === 'VariableDeclaration' ||
    data.type === 'IfStatement' ||
    data.type === 'ForStatement' ||
  ...
  return (
    ...
    >
      <Handle
        ...
        className="w-1.5 h-1.5 !bg-info opacity-40"
      />
