...
export function IRViewer() {
  ...
  const layoutWorker = useMemo(() => {
    return new WorkerManager<IRProgram, IRLayoutOutput>(
      () =>
        new Worker(
          new URL('../../workers/irLayout.worker.ts', import.meta.url),
          { type: 'module' },
        ),
    )
  }, [])
      ...
      {isBeginner && (
        <div className="p-4 bg-bg-tertiary border-b border-border-primary text-xs leading-relaxed text-text-secondary shrink-0 z-10">
          <h3 className="font-bold text-info mb-1 uppercase tracking-wider text-[10px]">
            What is Intermediate Representation (IR) & CFG?
          </h3>
          <p>
            The compiler translates the AST into a platform-neutral, simplified
            language called <strong>Intermediate Representation (IR)</strong>.
            It groups instructions into <strong>Basic Blocks</strong> (linear
            sequences of code with a single entry and exit point). These blocks
            are linked together to form a{' '}
            <strong>Control Flow Graph (CFG)</strong>, showing all possible
            execution paths (loops, branches).
          </p>
        </div>
      )}
      <div className="flex-1 relative min-h-0">
        {isCalculating && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-bg-primary/80 backdrop-blur-sm">
            <span className="text-info font-mono text-sm animate-pulse">
              Calculating CFG Layout...
            </span>
          </div>
        )}
        <ReactFlow
