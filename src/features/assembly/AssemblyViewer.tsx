...
const ASM_GLOSSARY: Record<string, string> = {
  ...
  'imull': 'Multiplies destination by source value and stores result in destination.',
  'cltd': 'Sign-extends %eax register into %edx, preparing for division.',
  'idivl': 'Divides %edx:%eax by source value; quotient in %eax, remainder in %edx.',
    ...
    <div className="flex flex-col h-full w-full bg-bg-primary font-mono text-sm">
      ...
      {isBeginner && (
        <div className="p-4 bg-bg-tertiary border-b border-border-primary text-xs leading-relaxed text-text-secondary shrink-0">
          <h3 className="font-bold text-info mb-1 uppercase tracking-wider text-[10px]">What is Assembly Generation & Register Allocation?</h3>
          <p>
            The <strong>Code Generator</strong> produces the final human-readable <strong>CPU Assembly Language</strong>. 
            Crucially, it maps abstract temporary variables from intermediate representation (IR) to actual hardware storage 
            slots called <strong>Registers</strong> (like <code>%eax</code>, <code>%ebx</code>) in a process called <strong>Register Allocation</strong>.
          </p>
        </div>
      )}
          ...
          {assembly.map((inst, idx) => {
            ...
            return (
              <div 
                ...
                title={isBeginner ? (ASM_GLOSSARY[inst.op] || `${inst.op} instruction`) : `Instruction ID: ${inst.id}`}
              >
                {/* Line number */}
