    ...
    <div className="flex flex-col h-full w-full bg-bg-primary font-mono text-sm">
      ...
      {isBeginner && (
        <div className="p-4 bg-bg-tertiary border-b border-border-primary text-xs leading-relaxed text-text-secondary shrink-0">
          ...
          <p>
            The <strong>Semantic Analyzer</strong> validates the AST for logical
            consistency. While the parser ensures C code is grammatically
            correct, semantic analysis checks rules like type matching and
            variable declaration availability. It compiles names into a
            <strong> Symbol Table</strong> and tracks variable visibility across
            nested <strong>Scopes</strong> (Global vs Local).
          </p>
        </div>
      )}
          ...
          <div className="w-full">
            ...
            <div className="space-y-1">
              {Object.values(symbols).map((sym) => (
                ...
                >
                  <div className="w-1/4 font-semibold text-info truncate">
                    {sym.name}
                  </div>
                  <div className="w-1/4 text-xs text-warning uppercase truncate">
                    {sym.symbolType}
                  </div>
                  <div
                    className="w-1/4 text-xs text-text-muted truncate"
                    title={sym.scopeId}
                  >
                    {sym.scopeId.slice(0, 8)}
                  </div>
          ...
          <div className="w-full">
            <div className="space-y-4">
              {Object.values(scopes).map((scope) => (
                <div
                  key={scope.id}
                  className="bg-bg-tertiary border border-border-primary rounded p-3"
                >
                  <div className="flex justify-between items-center mb-3 pb-2 border-b border-border-primary/50">
                    <div className="text-sm font-semibold text-text-primary">
                      Scope {scope.id.slice(0, 8)}
                    </div>
                    {scope.parentScopeId ? (
                      <div className="text-xs text-text-muted">
                        Parent: {scope.parentScopeId.slice(0, 8)}
                      </div>
                    ) : (
                      <div className="text-xs text-[#238636] font-bold px-2 py-0.5 rounded bg-[#238636]/10 border border-[#238636]/20">
                        GLOBAL
                      </div>
                    )}
                  </div>
                    ...
                    {scope.symbolIds.length > 0 ? (
                      scope.symbolIds.map((symId) => (
                        <div
                          ...
                          onMouseEnter={() =>
                            handleMouseEnter(symbols[symId].declarationNodeId)
                          }
                          onMouseLeave={handleMouseLeave}
                        >
                          ...
                          <span className="text-warning text-[10px] uppercase">
                            {symbols[symId].symbolType}
                          </span>
                        </div>
                      ))
                    ) : (
