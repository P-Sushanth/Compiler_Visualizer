...
describe('WorkerManager', () => {
  ...
  it('should cancel previous pending request when a new one is sent', async () => {
    ...
    // Resolve the second promise to avoid unused warning and fully verify cancellation flow
    const sentRequest2 = mockWorker.postMessage.mock.calls[1][0]
    mockWorker.onmessage({
      data: {
        requestId: sentRequest2.requestId,
        success: true,
        data: 'second-output',
        error: null,
      },
    })
    await expect(promise2).resolves.toBe('second-output')
  })
  ...
  it('should handle worker.onerror by rejecting pending promises', async () => {
    ...
    mockWorker.onerror(
      new ErrorEvent('error', { message: 'Web worker crashed' }),
    )
