...
class PipelineOrchestrator {
  ...
  private optimizerWorker: WorkerManager<
    import('@/types/compiler').IRProgram,
    OptimizerOutput
  >
  private assemblyWorker: WorkerManager<
    import('@/types/compiler').IRProgram,
    AssemblyOutput
  >
  ...
  constructor() {
    this.lexerWorker = new WorkerManager(
      () =>
        new Worker(new URL('../workers/lexer.worker.ts', import.meta.url), {
          type: 'module',
        }),
    )
    ...
    this.semanticWorker = new WorkerManager(
      () =>
        new Worker(new URL('../workers/semantic.worker.ts', import.meta.url), {
          type: 'module',
        }),
    )
    this.irWorker = new WorkerManager(
      () =>
        new Worker(new URL('../workers/ir.worker.ts', import.meta.url), {
          type: 'module',
        }),
    )
    this.optimizerWorker = new WorkerManager(
      () =>
        new Worker(new URL('../workers/optimizer.worker.ts', import.meta.url), {
          type: 'module',
        }),
    )
    this.assemblyWorker = new WorkerManager(
      () =>
        new Worker(new URL('../workers/assembly.worker.ts', import.meta.url), {
          type: 'module',
        }),
    )
  }
  ...
  public async compile(sourceCode: string) {
    ...
    try {
      ...
      if (signal.aborted) throw new Error('Cancelled')
      const assemblyResult = await this.assemblyWorker.execute(
        optimizerResult.ir,
      )
