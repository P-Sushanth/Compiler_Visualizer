  ...
  public execute(payload: TPayload): Promise<TData> {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        this.pendingRequests.has(this.latestRequestId)
      ) {
        const previousHandlers = this.pendingRequests.get(this.latestRequestId)
        if (previousHandlers) {
          clearTimeout(previousHandlers.timeoutId)
          previousHandlers.reject(
            new Error('Request cancelled by newer request'),
          )
      ...
      const timeoutId = setTimeout(() => {
        if (this.pendingRequests.has(requestId)) {
          ...
          if (handlers) {
            handlers.reject(
              new Error('Worker execution timed out (exceeded 5000ms)'),
            )
            this.pendingRequests.delete(requestId)
          }
          console.warn(
            `Worker timed out on request ${requestId}. Recovering worker thread.`,
          )
          if (this.worker) {
            this.worker.terminate()
